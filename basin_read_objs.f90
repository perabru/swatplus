...
!!     and reservoirs
      ...
      print *, "DEBUG-ALF: in_sim%object_cnt =", in_sim%object_cnt
      ...
      close (107)
      if (.not. allocated(ob)) allocate (ob(sp_ob%objs))
      if (.not. allocated(obcs)) allocate (obcs(sp_ob%objs))
      if (.not. allocated(obom)) allocate (obom(sp_ob%objs))
      return    
